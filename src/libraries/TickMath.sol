...
/// @dev implementation from: https://github.com/Uniswap/v4-core/blob/main/src/libraries/TickMath.sol
...
library TickMath {
	...
	function getSqrtRatioAtTick(int24 tick) internal pure returns (uint160 sqrtRatioX96) {
		unchecked {
			...
			sqrtRatioX96 = uint160((ratio >> 32) + (ratio % (1 << 32) == 0 ? 0 : 1));
		}
	}
	...
	function getTickAtSqrtRatio(uint160 sqrtRatioX96) internal pure returns (int24 tick) {
		unchecked {
			...
			tick = tickLow == tickHi ? tickLow : getSqrtRatioAtTick(tickHi) <= sqrtRatioX96
				? tickHi
				: tickLow;
		}
	}
